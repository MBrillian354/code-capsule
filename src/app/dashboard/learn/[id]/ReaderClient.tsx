...
export default function ReaderClient({ capsule }: { capsule: CapsuleClient }) {
  ...
  const initialPage = clamp(
    initialFromQuery || saved?.last_page_read || capsule.last_page_read || 1,
    1,
    totalPages
  );
  ...
  const [progress, setProgress] = React.useState<number>(
    computeProgress(
      Math.max(saved?.last_page_read || 0, page),
      totalPages
    )
  );
  ...
  React.useEffect(() => {
    ...
    setSavedProgress(capsule.id, { last_page_read: newLast, overall_progress: newProgress });
  ...
  const current = capsule.content.find((c) => c.page === page) || capsule.content[0];
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2, pb: 6 }}>
      ...
      <Box
        sx={{
          position: "sticky",
          top: 64, // assumes app bar height; tweak to your layout
      ...
      >
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Page {page} of {totalPages} • {Math.round(progress)}%
          </Typography>
        </Box>
      ...
      <Box sx={{ px: { xs: 2, md: 3 }, pt: 1 }}>
        <article
          ...
          dangerouslySetInnerHTML={{ __html: marked.parse(current.body) as string }}
          style={{
            lineHeight: 1.7,
            fontSize: "1.05rem",
          }}
