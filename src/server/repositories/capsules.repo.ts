...
export async function insertCapsule(params: {
  ...
  await sql`INSERT INTO capsules (id, title, total_pages, content, created_by) VALUES (${id}, ${params.title}, ${params.total_pages}, ${sql.json(params.content as unknown as JSONValue)}, ${params.created_by})`
  return { id }
}

export async function getCapsule(capsuleId: string) {
  const rows = await sql`SELECT id, title, total_pages, content, created_by, created_at FROM capsules WHERE id = ${capsuleId}`
  return rows[0] ?? null
}
...
export async function getCapsuleWithCreator(capsuleId: string) {
  const rows = await sql`SELECT c.id, c.title, c.total_pages, c.created_at, c.content, c.created_by, u.name as creator_name FROM capsules c LEFT JOIN users u ON c.created_by = u.id WHERE c.id = ${capsuleId}`
  return rows[0] ?? null
}
